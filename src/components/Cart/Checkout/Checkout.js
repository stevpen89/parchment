...
class Checkout extends Component {
	...
	componentDidMount () {
		...
		if (hasShipping === true){this.setState({containsJournal:true})}
		this.countOrders()
		let taxrate = .0575
		let totalWithTax = total*(1 +taxrate)
		this.setState({totalWithTax,tax:(total*taxrate)})
	}
				...
				<div className="checkout">
					...
					<div>
						<select onChange={(e)=>this.handleSelect(e.target.value)}>
							...
							<option value='Arkansas'>Arkansas</option>							<option value='Arizona'>Arizona</option>
